...
const FeedbackRadio: React.FC< { question:FeedbackItemModel, empty:boolean, options:string[], active?:number, onResponse: (response: {
    feedback_result: JSON;
    question: FeedbackItemModel;
}) => void }> = ({question, empty, options, active, onResponse }) => {
    ...
    return  <div key={index}>
                <input type="radio" name="radial" id={"option"+index.toString()} value={index} onChange={() => handleRadioClick(index)}/>
                <label className="eaves book font fs18 purple-dark" htmlFor={"option"+index.toString()}>{option}</label>
            </div>
    }
    ): active? <div key={active}>
            <input type="radio" name="radial" id={"option" + active.toString()} value={active} onChange={() => handleRadioClick(index)}/>
